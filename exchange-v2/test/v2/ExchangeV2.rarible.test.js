...
contract("ExchangeV2, sellerFee + buyerFee =  6%,", accounts => {
  ...
  describe("estimate gas", () => {
    //toDO test 1155
    //toDO test acceptBid
    it("Direct buy with order Data 721<->ETH", async () => {
      const seller = accounts[2];
      const buyer = accounts[1];
      const buyOriginAddr = accounts[3];
      const sellOriginAddr = accounts[4];
      const buyPayoutAddr = accounts[5];
      const sellPayoutAddr = accounts[6];
      const price = 10;
      const salt = 1;
      const tokenAmount = 1
      ...
      let buyOriginPair = await transferManagerTest.encodeOriginFeeIntoUint(buyOriginAddr, 0); //addr + value%
      let sellOriginPair = await transferManagerTest.encodeOriginFeeIntoUint(sellOriginAddr, 0);
      let buyPayoutPair = await transferManagerTest.encodeOriginFeeIntoUint(buyer, 10000);
      let sellPayoutPair = await transferManagerTest.encodeOriginFeeIntoUint(seller, 10000);
			const encDataLeft = await encDataV2([[[seller, 10000]], [[sellOriginAddr, 0]], true]);

			const left = Order(seller, Asset(ERC721, enc( erc721.address, erc721TokenId1), 1), ZERO, Asset(ETH, "0x", price), salt, 0, 0, ORDER_DATA_V2, encDataLeft);
      const signature = await getSignature(left, seller);
      var ordersDataArray = [sellPayoutPair, buyPayoutPair, sellOriginPair, buyOriginPair];
      var testObject = {seller: seller, token: erc721.address, assetType: ERC721, tokenId: erc721TokenId1, tokenAmount: tokenAmount, price: price, salt: salt, signature: signature};
      const tx = await testing.directBuy(testObject, ordersDataArray, { from: buyer, value: 100 });
      console.log("Direct buy with order Data  721<->ETH gas used:", tx.receipt.gasUsed);
		})

    it("Old Style buy 721<->ETH", async () => {
      const seller = accounts[2];
      const buyer = accounts[1];
      const price = 10;
      const salt = 1;
			...
			const left = Order(seller, Asset(ERC721, enc( erc721.address, erc721TokenId1), 1), ZERO, Asset(ETH, "0x", price), salt, 0, 0, ORDER_DATA_V2, encDataLeft);
			const right = Order(buyer, Asset(ETH, "0x", price), ZERO, Asset(ERC721, enc( erc721.address, erc721TokenId1), 1), salt, 0, 0, "0xffffffff", "0x");
      const signature = await getSignature(left, seller);
      const tx = await testing.matchOrders(left, signature, right, "0x", { from: buyer, value: 100 });
      console.log("oldStyle buy 721<->ETH gas used:", tx.receipt.gasUsed);
